...
from iOpt.models.model_mlp import ModelMLPProba
    ...
    with open(filename, "w") as f:
        ...
        for func_1, func_2 in func_ids:
            ...
            for target_eps in [0.01]:
                ...
                #     print("a", alpha)
                #     model = ModelLinearSVCprobaLogNorm()
                #     hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2,
                ...
                # for i, alpha in enumerate((0.03, 0.09)):
                #     print("a", alpha)
                #     model = ModelPolySVCprobaLogNorm()
                #     hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2,
                #                                                    alpha=alpha, model=model, eps=target_eps,
                ...
                model = ModelNNProba(weights=[0.2, 0.8])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.01,
                                                               model=model, eps=target_eps, name="(a) NN [0.2, 0.8], a=0.01", axes=axes,
                                                               ax1=0, ax2=0)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                model = ModelNNProba(weights=[0.1, 0.9])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.01,
                                                               model=model, eps=target_eps, name="(a) NN [0.1, 0.9], a=0.01", axes=axes,
                                                               ax1=0, ax2=1)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                model = ModelNNProba(weights=[0.05, 0.85])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.01,
                                                               model=model, eps=target_eps, name="(a) NN [0.05, 0.85], a=0.01", axes=axes,
                                                               ax1=1, ax2=0)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                model = ModelNNProba(weights=[0.01, 0.99])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.01,
                                                               model=model, eps=target_eps, name="(a) NN [0.01, 0.99], a=0.01", axes=axes,
                                                               ax1=1, ax2=1)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)
                ...
                model = ModelNNProba(weights=[0.2, 0.8])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.1,
                                                               model=model, eps=target_eps, name="(a) NN [0.2, 0.8],  a=0.1", axes=axes,
                                                               ax1=2, ax2=0)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                model = ModelNNProba(weights=[0.1, 0.9])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.1, model=model, eps=target_eps, name="(a) NN [0.1, 0.9],  a=0.1",
                                                               axes=axes,
                                                               ax1=2, ax2=1)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)
                ...
                model = ModelNNProba(weights=[0.05, 0.85])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.1,
                                                               model=model, eps=target_eps, name="(a) NN [0.05, 0.85],  a=0.1", axes=axes,
                                                               ax1=3, ax2=0)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                model = ModelNNProba(weights=[0.01, 0.99])
                hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.1,
                                                               model=model, eps=target_eps, name="(a) NN [0.01, 0.99],  a=0.1", axes=axes,
                                                               ax1=3, ax2=1)
                print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)

                # model = ModelMLPProba()
                # hw_index, iter_count = calculate_grishagin_mco(func_id_1=func_1, func_id_2=func_2, alpha=0.05, model=model, eps=target_eps,
                #                                                name="(a) MLP", axes=axes, ax1=0, ax2=1)
                # print("v, e =", target_eps, "alpha =", 0.06, hw_index, iter_count)
                f.flush()
                plt.show()
        time2 = time.time()
